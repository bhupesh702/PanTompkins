# -*- coding: utf-8 -*-
"""
Created on Wed May 31 15:53:30 2023
...
def PanTompkins(input_val):
    ...
    if integral[current] >= threshold_i1 and highpass[current] >= threshold_f1:
        if sample > lastQRS + FS/5:
            ...
            if sample > DELAY + BUFFSIZE:
                output_s.append(outputSignal[0])
    ...
    for i in range(1, BUFFSIZE):
        output_s.append(outputSignal[i])
    
    
data=pd.read_csv('103.csv')
column=data.iloc[0:150000,2]
...
curr_ind=0
next_ind=1
samplenumber=1 #CHANGE TO 0 FOR BIH DATASET
row=0
col=0
...
peaks=find_peaks(integral_s,distance=160)
#print('Using find_peaks() with distance=160',peaks)
...
plt.plot(output_s)
plt.xlabel('Sample Number')
plt.ylabel('Final o/p')
plt.title('FINAL PLOT AFTER THRESHOLDING')
